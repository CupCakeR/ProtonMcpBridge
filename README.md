...
```sh
claude mcp add --transport http sbox http://127.0.0.1:7270/mcp
```

## Config

**View > MCP Bridge** has a status light and a Start/Stop button. The enabled state and port are
saved per-user and persist across restarts.
...
mcp_bridge_port 7300  change the port (default 7270)
